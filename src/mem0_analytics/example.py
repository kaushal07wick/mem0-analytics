...
# ---- disable analytics autopatch BEFORE import ----
os.environ.setdefault("MEM0_ANALYTICS_ENABLED", "false")

# import patch + aggregator only (no autopatch)
...
def build_config(provider: str, model: str, collection_name: str) -> dict:
    ...
    if provider == "openai":
        ...
        vector_store = {
            "provider": "chroma",
            "config": {"path": "./chroma_store", "collection_name": collection_name},
        }
    return {
...
def make_memory(args):
    ...
    mem = Memory.from_config(cfg)              # construct first (no autopatch)
    mem = patch_memory(mem)                    # patch instance only (safe)
    return mem
...
def main():
    # start aggregator in background (once)
    start_background_aggregator()
    print("[mem0-analytics] auto-tracking disabled (instance-patch mode)")
    print("[aggregate] background KPI aggregator started ✅")
